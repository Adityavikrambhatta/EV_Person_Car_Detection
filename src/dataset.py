...
class COCODataset(Dataset):
    ...
    def __generatelabeltxt__(self, mode):
        ...
        label_dir = f'{self.root_dir}/data_images/labels/{mode}/'
        img_list = os.listdir(f'{self.root_dir}/data_images/images/{mode}/')
        
        print(label_dir)
        if not os.path.exists(label_dir):
            os.makedirs(label_dir)
            images = self.images
            images = [ image for image in images if image['file_name'] in img_list ]
...
def create_datasets(cfg, mode):
    ...
    utils.create_train_val_split(cfg['images'], f"{wk_dir}/data_images/images", 0.8)
 
    train_dataset = dataset.__generatelabeltxt__( 'train') 
    val_dataset = dataset.__generatelabeltxt__( 'val')
